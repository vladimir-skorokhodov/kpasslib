...
void main() {
  group('KDBX database tests', () {
    ...
    test('creates new database', () {
      ...
      final subGroup = db.createGroup(parent: db.root, name: 'subgroup');
    ...
    test('deletes and restores an entry', () {
      ...
      expect(parent.groups.length, groupLength - 1);

      db.move(item: group, target: parent);
    ...
    test('imports an entry from another file', () {
      ...
      final importedEntryWithCustomIcon = db.importEntry(
        entry: sourceEntryWithCustomIcon,
        target: db.root,
        other: sourceDb,
      );
      final importedEntryWithBinaries = db.importEntry(
        entry: sourceEntryWithBinaries,
        target: db.root,
        other: sourceDb,
      );
    ...
    test('changes password', () {
      ...
      final credentials2 = KdbxCredentials(
      ...
      expect(db.meta.generator, 'KPassLib');
      expect(db.header.credentials.hash, credentials2.hash);
      expect(_getTestDb().isEqual(db), true);
    });
  });
}
